...
def plot_kl_diff(kl_diff, N_LAYERS, N_HEADS, k):
    ...
    plt.savefig(f"figures/top_{k}_attention_head_ablation_kl_diff_llama3b.png")
    plt.savefig(f"figures/top_{k}_attention_head_ablation_kl_diff_llama3b.eps")
...
def average_residual_stream_patching(N_LAYERS, model_name, dataset):
    ...
    for idiom_entry in dataset:
        ...
        for pair in pairs:
            ...
            for answer_idx in range(3):
                ...
                # # Extract the actual tensor value from the saved Proxy
                # clean_logit_diff_value = clean_logit_diff.value.item()
                ...
                # # Extract the actual tensor value from the saved Proxy
                # corrupted_logit_diff_value = corrupted_logit_diff.value.item()
...
def run_average_residual_stream_patching(dataset, N_LAYERS, model_name):
    
        total_results, counts, total_kl, kl_counts = average_residual_stream_patching(N_LAYERS, model_name, dataset)
        ...
        plot_patching_kl_heatmap(
            ...
            "figures/averaged_residual_stream_patching_kl_multiple_llama3b.png",
            "figures/averaged_residual_stream_patching_kl_multiple_llama3b.eps",
        ...
        plot_kl_vs_layer_line(
            avg_kl,
            "Dataset-Averaged KL(clean || patched), Residual Stream Patching",
            "figures/averaged_residual_stream_patching_kl_multiple_llama3b_line.png",
            "figures/averaged_residual_stream_patching_kl_multiple_llama3b_line.eps",
        )
        return plt.gcf()
...
def run_average_attention_head_patching(dataset, N_LAYERS, N_HEADS, D_HEADS, model_name):
    average_patching_results, avg_kl = average_attention_head_patching(N_LAYERS, N_HEADS, D_HEADS, dataset, model_name)
    x_labels = [f"Head {i+1}" for i in range(N_HEADS)]
    ...
    plot_patching_kl_heatmap(
        ...
        "figures/averaged_attention_head_patching_kl_multiple_llama3b.png",
        "figures/averaged_attention_head_patching_kl_multiple_llama3b.eps",
    ...
    plot_kl_vs_layer_line(
        ...
        "figures/averaged_attention_head_patching_kl_multiple_llama3b_line.png",
        "figures/averaged_attention_head_patching_kl_multiple_llama3b_line.eps",
    )
    return plt.gcf()
