...
def get_first_head_tokens_from_layers(model_name, model, tokeniser, input_ids, attention_mask, corrected_form_compounds_per_sentence, layers, load_if_available=True, batch_size = 1,  rep_loc='./data', rep_type="first_head_context_combined", torch_device="cuda", save_attention=False):
    return get_tokens_from_layers(data_loc=rep_loc, model_name=model_name, model=model, tokeniser=tokeniser, input_ids=input_ids, attention_mask=attention_mask, layers=layers,
...
def get_first_mod_tokens_from_layers(model_name, model, tokeniser, input_ids, attention_mask, corrected_form_compounds_per_sentence, layers, load_if_available=True, batch_size = 1,  rep_loc='./data', rep_type="first_modifier_context_combined", torch_device="cuda", save_attention=False):
    return get_tokens_from_layers(data_loc=rep_loc, model_name=model_name, model=model, tokeniser=tokeniser, input_ids=input_ids, attention_mask=attention_mask, layers=layers,
...
def get_b_word_token_from_layers(model_name, model, tokeniser, input_ids, attention_mask, corrected_form_compounds_per_sentence_bs, layers, load_if_available=True, batch_size = 1, rep_type="b_word_context_combined", rep_loc='./data', torch_device="cuda", save_attention=False):
    return get_tokens_from_layers(data_loc=rep_loc, model_name=model_name, model=model, tokeniser=tokeniser, input_ids=input_ids, attention_mask=attention_mask, layers=layers,
...
def b_word_selector(model, model_name, tokeniser, token_reps, input_ids, layer, batch_size, i, corrected_form_compounds_per_sentence_bs):
    ...
    compounds_per_batch = np.array([(' ' + x[0], ' ' + x[1]) for x in corrected_form_compounds_per_sentence_bs])[i:i+batch_size]
    ...
    final_word_input_ids_per_sent = [np.array(x)[get_tokens_to_keep(x)] for x in final_word_input_ids_per_sent_raw]
    final_word_locs_per_sent = [search_sequence_numpy(input_ids[i, :].cpu().numpy().reshape(-1), x.reshape(-1)) for i, x in enumerate(final_word_input_ids_per_sent)]
    bs_reps = np.vstack([reps[final_word_locs_per_sent[i][-1]].cpu().numpy() for i, reps in enumerate(layer_reps)])
    return bs_reps
...
def extract_and_save_representations(amount_of_dataset=1, batch_size=1, models=None, load_if_available=False, layers=None, torch_device="cpu", representations=["mean_pooled"], rep_loc='./data', save_attention=False):
    ...
    b1_b2_words_per_sentence_with_context = data_utils.get_idiom_modifier_head_words_per_sentence_context_bs()
    ...
    for model_name in tqdm.tqdm(models):
        ...
        if "final_head_context" in representations:
            ...
            #input_ids, attention_mask = unpack_dict(tokeniser.batch_encode_plus(sentences.tolist(), max_length=512, return_tensors='pt', pad_to_max_length=True))
            corrected_form_compounds_per_sentence = data_utils.load_correct_form_context()
            get_final_head_tokens_from_layers(model_name, model, tokeniser, input_ids, attention_mask, corrected_form_compounds_per_sentence, layers=layers, load_if_available=load_if_available, batch_size=batch_size, rep_type='final_head_context_v2', rep_loc=rep_loc, torch_device=torch_device, save_attention=save_attention)
        ...
        if "final_word_context" in representations:
            ...
            get_final_word_token_from_layers(model_name, model, tokeniser, input_ids, attention_mask, corrected_form_compounds_per_sentence_and, layers=layers, load_if_available=load_if_available, batch_size=batch_size, rep_type='final_word_context_v2', rep_loc=rep_loc, torch_device=torch_device, save_attention=save_attention)
