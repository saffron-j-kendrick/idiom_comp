...
def get_first_head_tokens_from_layers(model_name, model, tokeniser, input_ids, attention_mask, corrected_form_compounds_per_sentence, layers, load_if_available=True, batch_size = 1,  rep_loc='./data', rep_type="first_head_no_context", torch_device="cuda", save_attention=False):
    return get_tokens_from_layers(data_loc=rep_loc, model_name=model_name, model=model, tokeniser=tokeniser, input_ids=input_ids, attention_mask=attention_mask, layers=layers,
...
def get_first_mod_tokens_from_layers(model_name, model, tokeniser, input_ids, attention_mask, corrected_form_compounds_per_sentence, layers, load_if_available=True, batch_size = 1,  rep_loc='./data', rep_type="first_modifier_no_context", torch_device="cuda", save_attention=False):
    return get_tokens_from_layers(data_loc=rep_loc, model_name=model_name, model=model, tokeniser=tokeniser, input_ids=input_ids, attention_mask=attention_mask, layers=layers,
...
def get_b_word_token_from_layers(model_name, model, tokeniser, input_ids, attention_mask, corrected_form_compounds_per_sentence_bs, layers, load_if_available=True, batch_size = 1, rep_type="b_word_no_context", rep_loc='./data', torch_device="cuda", save_attention=False):
    return get_tokens_from_layers(data_loc=rep_loc, model_name=model_name, model=model, tokeniser=tokeniser, input_ids=input_ids, attention_mask=attention_mask, layers=layers,
...
def extract_and_save_representations(amount_of_dataset=1, batch_size=1, models=None, load_if_available=False, layers=None, torch_device="cuda", representations=["mean_pooled"], rep_loc='./data', save_attention=False):
    ...
    if representations == None:
        representations = ["final_modifier_no_context"]
    ...
    for model_name in tqdm.tqdm(models):
        ...
        if "b_word_no_context" in representations:
            ...
            attention_mask = inputs["attention_mask"]
            #input_ids, attention_mask = unpack_dict(tokeniser.batch_encode_plus(sentences.tolist(), max_length=512, return_tensors='pt', pad_to_max_length=True))
            corrected_form_compounds_per_sentence_bs = data_utils.load_correct_form_no_context_bs()
            get_b_word_token_from_layers(model_name, model, tokeniser, input_ids, attention_mask, corrected_form_compounds_per_sentence_bs, layers=layers, load_if_available=load_if_available, batch_size=batch_size, rep_type='b_word_no_context', rep_loc=rep_loc, torch_device=torch_device, save_attention=save_attention)
